...
The_class_of_the_account::
The_class_of_the_account(
        const unsigned int& new_account,
...
The_class_of_the_account(
        const unsigned int& new_account,
        const unsigned int& new_date,
        const size_t& new_files){
    the_latest_date = new_date;
    the_account_of_the_broker = new_account;
    the_number_of_files = new_files;
}

void The_class_of_the_account::
update_the_information_of_the_account(
        const unsigned int& new_date,
...
bool The_class_of_the_account::
operator==(const The_class_of_the_account& new_account) const {
    return
    the_account_of_the_broker ==
    new_account.the_account_of_the_broker &&
    the_number_of_files ==
    new_account.the_number_of_files &&
    the_latest_date ==
    new_account.get_the_latest_date();
}
...
unsigned int The_class_of_the_account::
get_the_latest_date() const {
    return the_latest_date;
}

size_t The_class_of_the_account::
get_the_number_of_files() const {
    return the_number_of_files;
}

unsigned int The_class_of_the_account::
get_the_id_of_the_broker() const {
    return the_account_of_the_broker;
}

std::map<std::string, std::set<The_class_of_the_account>>
the_mainlist_of_brokers_accounts(
        const filesys::path& new_path) {
    std::map<std::string,
    std::set<The_class_of_the_account>> the_information;
    for (const auto& the_entry_to_directory :
    filesys::directory_iterator(new_path)) {
        if (!filesys::is_directory(the_entry_to_directory)){
            continue;
        }
        const auto& the_entry_to_the_path =
                the_entry_to_directory.path();
        auto the_name_of_entry =
                the_entry_to_the_path.filename().string();
        the_information.insert(
                std::make_pair(the_name_of_entry,
                        the_lists_of_brokers_accounts(
                                the_entry_to_the_path).first));
    }
...
the_lists_of_brokers_accounts(const filesys::path& new_path) {
    const std::regex the_template_of_filename(
            "^balance_(\\d{8})_(\\d{8}).txt$");
    ...
    filesys::directory_iterator(new_path)) {
        ...
        std::string the_name_of_the_file =
                the_entry_to_the_path.filename().string();
        if (!std::regex_match(the_name_of_the_file,
                the_template_of_filename)) {
            continue;
        }
        ...
        The_class_of_the_account an_account(
                std::stoi(the_match[1]),
                std::stoi(the_match[2]));
        auto the_accounts_iter =
                the_set_of_the_accounts.find(an_account);
    ...
    return std::make_pair(
            the_set_of_the_accounts,
            the_list_of_the_accounts);
}

bool check_if_data_is_correct(
        const std::string& new_date) {
